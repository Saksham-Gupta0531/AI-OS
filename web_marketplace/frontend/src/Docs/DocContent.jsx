...
export default function DocContent({ category, articleId }) {
  ...
  // If no article is selected, render the Category Index (Kali style)
  if (!articleId) {
    return (
  ...
  // Fetch Markdown if article IS selected
  useEffect(() => {
    setLoading(true);
    // Combines category and article ID to fetch the file (e.g. /docs/dev-codecheater.md)
